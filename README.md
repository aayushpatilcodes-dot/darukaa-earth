...
services (Render for the API, Aiven for Postgres, Vercel for the static frontend) — see
[Deployment](#deployment).
...
| Deployment     | Render (API) / Aiven (Postgres) / Vercel (frontend) | Permanent free tiers, no card required, all auto-deploy from GitHub             |
...
**Database → Aiven** (no card, doesn't expire — Render's own free Postgres is deleted after 30 days):

1. Create a free account at [aiven.io](https://aiven.io) (no payment details required) and create a
   free PostgreSQL service with the PostGIS extension enabled.
2. Copy its connection string (`postgresql://...`) for the next step.

**Backend → Render**, using `render.yaml` (Render "Blueprint"):

1. Push this repo to GitHub and create a new Render Blueprint pointing at it (no card required for
   the free plan).
2. Render provisions a Docker web service built from `backend/Dockerfile`. When prompted, paste the
   Aiven connection string as `DATABASE_URL` (it isn't auto-generated, since the database lives
   outside Render).
3. `JWT_SECRET_KEY` is auto-generated by Render. Update `CORS_ORIGINS` to your actual Vercel URL
   once you have it.
4. The container's `CMD` runs `alembic upgrade head` before starting `uvicorn`, so every deploy
   migrates the database (and enables the `postgis` extension) automatically.
